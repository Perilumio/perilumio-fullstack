#!/usr/bin/env node
// Generiert eine idempotente Supabase-Migration für den Kurs Informatiker
// (Informatiker/in EFZ) auf Basis der Bildungsplan-CSV
...
//     {abu, automechaniker, fage, informatiker, kv, schreiner, strassenbau}
//     erweitern.
//   * Ein Modul mit course_key 'informatiker' wird nach (course_key, title)
...
//
// Eingabe: supabase/seeds/informatiker_fragenkatalog_bildungsplan.csv
// Ausgabe: supabase/migrations/20260531_informatiker_bildungsplan.sql
...
const COURSES = [
  {
    course_key: 'informatiker',
    csv: 'supabase/seeds/informatiker_fragenkatalog_bildungsplan.csv',
    moduleTitle: 'Informatiker/in EFZ – Bildungsplan',
    moduleDescr: 'Bildungsplan Informatiker/in EFZ nach BiVo 2021 (ICT-Berufsbildung Schweiz/SBFI). 16 Lektionen entlang der gemeinsamen Handlungskompetenzbereiche HKB a–c und der Fachrichtungen Applikationsentwicklung sowie Plattformentwicklung mit Fokus auf ICT-Projekte, Daten/Datenbanken, Security, Support, Software-Engineering, Tests, Deployment, Netzwerk-, Server- und Cloud-Infrastruktur.',
    modulePosition: 100,
  },
];

const ALLOWED_KEYS = ['abu', 'automechaniker', 'fage', 'informatiker', 'kv', 'schreiner', 'strassenbau'];

const out = [];
out.push('-- Informatiker (Informatiker/in EFZ): Bildungsplan-Fragenkatalog (16 × 30 Fragen).');
out.push('-- Idempotent, additiv und ohne Datenverlust:');
out.push('--   * course_key-CHECK-Constraints werden auf');
out.push('--     abu/automechaniker/fage/informatiker/kv/schreiner/strassenbau erweitert.');
...
const outputPath = 'supabase/migrations/20260531_informatiker_bildungsplan.sql';
