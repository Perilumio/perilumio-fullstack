#!/usr/bin/env node
// Generiert eine idempotente Supabase-Migration für den Kurs Detailhandel
// (Detailhandelsfachfrau/Detailhandelsfachmann EFZ) auf Basis der Bildungsplan-CSV
...
//     {abu, automechaniker, detailhandel, fage, informatiker, kv, schreiner, strassenbau}
//     erweitern.
//   * Ein Modul mit course_key 'detailhandel' wird nach (course_key, title)
...
//
// Eingabe: supabase/seeds/detailhandel_fragenkatalog_bildungsplan.csv
// Ausgabe: supabase/migrations/20260532_detailhandel_bildungsplan.sql
...
const COURSES = [
  {
    course_key: 'detailhandel',
    csv: 'supabase/seeds/detailhandel_fragenkatalog_bildungsplan.csv',
    moduleTitle: 'Detailhandelsfachfrau/Detailhandelsfachmann EFZ – Bildungsplan',
    moduleDescr: 'Bildungsplan Detailhandelsfachfrau/Detailhandelsfachmann EFZ (BDS/SBFI). 16 Lektionen entlang der Handlungskompetenzbereiche zu Kundenbeziehungen, Warenbewirtschaftung, Produkt- und Dienstleistungskenntnissen, Betrieb und Branche sowie Einkaufserlebnissen inklusive Online-Shop.',
    modulePosition: 100,
  },
];

const ALLOWED_KEYS = ['abu', 'automechaniker', 'detailhandel', 'fage', 'informatiker', 'kv', 'schreiner', 'strassenbau'];

const out = [];
out.push('-- Detailhandel (Detailhandelsfachfrau/Detailhandelsfachmann EFZ):');
out.push('-- Bildungsplan-Fragenkatalog (16 × 30 Fragen).');
out.push('-- Idempotent, additiv und ohne Datenverlust:');
out.push('--   * course_key-CHECK-Constraints werden auf');
out.push('--     abu/automechaniker/detailhandel/fage/informatiker/kv/schreiner/strassenbau erweitert.');
...
const outputPath = 'supabase/migrations/20260532_detailhandel_bildungsplan.sql';
