#!/usr/bin/env node
// Generiert eine idempotente Supabase-Migration für den Kurs KV
// (Kauffrau/Kaufmann EFZ) auf Basis der Bildungsplan-CSV
...
//     {abu, automechaniker, fage, kv, schreiner, strassenbau} erweitern.
//   * Ein Modul mit course_key 'kv' wird nach (course_key, title)
...
//
// Eingabe: supabase/seeds/kv_fragenkatalog_bildungsplan.csv
// Ausgabe: supabase/migrations/20260530_kv_bildungsplan.sql
...
const COURSES = [
  {
    course_key: 'kv',
    csv: 'supabase/seeds/kv_fragenkatalog_bildungsplan.csv',
    moduleTitle: 'Kauffrau/Kaufmann EFZ – Bildungsplan',
    moduleDescr: 'Bildungsplan Kauffrau/Kaufmann EFZ nach BiVo 2023 (SBFI/SKKAB). 16 Lektionen entlang der Handlungskompetenzbereiche HKB A–E mit Fokus auf Rolle/Betrieb, Interaktion mit Anspruchsgruppen, kaufmännische Arbeitsprozesse, Lernen/Arbeitstechniken sowie wirtschaftliche/gesellschaftliche Zusammenhänge.',
    modulePosition: 100,
  },
];

const ALLOWED_KEYS = ['abu', 'automechaniker', 'fage', 'kv', 'schreiner', 'strassenbau'];

const out = [];
out.push('-- KV (Kauffrau/Kaufmann EFZ): Bildungsplan-Fragenkatalog (16 × 30 Fragen).');
out.push('-- Idempotent, additiv und ohne Datenverlust:');
out.push('--   * course_key-CHECK-Constraints werden auf');
out.push('--     abu/automechaniker/fage/kv/schreiner/strassenbau erweitert.');
...
const outputPath = 'supabase/migrations/20260530_kv_bildungsplan.sql';
