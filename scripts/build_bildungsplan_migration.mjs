#!/usr/bin/env node
// Generiert eine idempotente Supabase-Migration für die Kurse
// Automechaniker und Schreiner auf Basis der Bildungsplan-CSVs
...
//     {abu, automechaniker, schreiner, strassenbau} erweitern.
//   * Pro Kurs ein Modul mit course_key in {automechaniker, schreiner};
//     Module werden nach (course_key, title) gesucht oder angelegt.
...
//
// Eingabe: supabase/seeds/automechaniker_fragenkatalog_bildungsplan.csv
//          supabase/seeds/schreiner_fragenkatalog_bildungsplan.csv
// Ausgabe: supabase/migrations/20260528_automechaniker_schreiner_bildungsplan.sql
...
// Gruppiert die CSV-Zeilen pro (sub_hk + topic) zu Lektionen mit 30 Fragen.
// Position innerhalb der Lektion folgt der CSV-Reihenfolge (1..30).
function groupByLesson(rows) {
  ...
  // Lektionen werden nach sub_hk-Nummer (L01..L16) sortiert.
  return Array.from(map.values()).sort((a, b) => a.sub_hk.localeCompare(b.sub_hk, 'de'));
}

// Lesson-Titel: «L01 – Topic». Bleibt sprechend und stabil.
...
const COURSES = [
  {
    course_key: 'automechaniker',
    csv: 'supabase/seeds/automechaniker_fragenkatalog_bildungsplan.csv',
    moduleTitle: 'Automechaniker/in EFZ – Bildungsplan',
    moduleDescr: 'Bildungsplan Automechaniker/in EFZ. 16 Lektionen entlang der Handlungskompetenzbereiche HKB1–HKB5 mit Fokus auf Inspektion, Service, Antriebsstrang, Elektrik/Elektronik, Komfort- und Sicherheitssysteme sowie alternative Antriebe.',
    modulePosition: 100,
  },
  {
    course_key: 'schreiner',
    csv: 'supabase/seeds/schreiner_fragenkatalog_bildungsplan.csv',
    moduleTitle: 'Schreiner/in EFZ – Bildungsplan',
    moduleDescr: 'Bildungsplan Schreiner/in EFZ. 16 Lektionen entlang der Semester 1–8 mit Vorbereitung/Projektion (VP) und Holztechnologie/Maschinen (HM); Verbindungen, AVOR, CAD, Innenausbau, Brandschutz und QV-Vorbereitung.',
    modulePosition: 100,
  },
];

const ALLOWED_KEYS = ['abu', 'automechaniker', 'schreiner', 'strassenbau'];

const out = [];
out.push('-- Automechaniker + Schreiner: Bildungsplan-Fragenkatalog (16 × 30 Fragen).');
out.push('-- Idempotent, additiv und ohne Datenverlust:');
out.push('--   * course_key-CHECK-Constraints werden auf abu/automechaniker/schreiner/strassenbau erweitert.');
...
// Pro Kurs: Modul + 16 Lektionen × 30 Fragen.
for (const courseDef of COURSES) {
  const rows = loadCsv(courseDef.csv);
  // Sicherheit: alle Zeilen gehören zum erwarteten course_key.
  for (const r of rows) {
...
const outputPath = 'supabase/migrations/20260528_automechaniker_schreiner_bildungsplan.sql';
