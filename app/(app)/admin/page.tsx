...
export default async function AdminPage(){
  ...
  return <AppShell><section className="stack"><div className="card hero"><div><span className="pill">Admin</span><h1>Content- und Kursverwaltung</h1><p className="muted">Zugriff nur mit Admin-Rolle.</p></div><Lumio /></div><div><Link href="/admin/analytics" className="btn btn-primary">Analytics öffnen</Link></div><div className="grid grid-2"><AdminImportCard /><AdminImportHistory logs={logsResult.data ?? []} /></div><AdminQuestionTable questions={questions ?? []} /></section></AppShell>;
}
