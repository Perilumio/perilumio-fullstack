...
export default async function AdminPage(){
  ...
  return <AppShell><section className="stack"><div className="card hero"><div><span className="pill">Admin</span><h1>Content- und Kursverwaltung</h1><p className="muted">Zugriff nur mit Admin-Rolle.</p></div></div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><Link href="/admin/analytics" className="btn btn-primary">Analytics öffnen</Link><Link href="/admin/accounts" className="btn">Konten verwalten</Link></div><div className="grid grid-2"><AdminImportCard /><AdminImportHistory logs={logsResult.data ?? []} /></div><AdminQuestionTable questions={questions ?? []} /></section></AppShell>;
}
